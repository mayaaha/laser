    ...
    def intervene(self, model, tokenizer, dataset, args, llm_name):
        ...
        for i in tqdm(range(0, dataset_size)):
            ...
            with open("bbh_qa_results.txt", "a") as file1:
                    file1.write(f"{int(is_correct == True)}\t{generation.encode("utf-8")}\t{answer.encode("utf-8")}\t{prepared_answer.encode("utf-8")}\t{prepared_generation.encode("utf-8")}\t{prompt.encode("utf-8")}\n")
